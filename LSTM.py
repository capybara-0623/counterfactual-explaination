...
class Model(nn.Module):
    def __init__(self, vocab_size, embed_size, dropout, lstm_size, max_length):
        ...
        if isinstance(dropout, (list, tuple)):
            ...
            # 限制在 0~1 之间（防止超范围）
        self.lstm_dropout = max(0.0, min(1.0, dropout))
        self.lstm_size = 2  # 固定层数，防止缺失报错
        # ======================================================
        ...
        if isinstance(self.vocab_size, (list, tuple)):
            ...
            # 兜底：如果不小心变成数字，也不会报错
            self.embed_act = nn.Embedding(self.vocab_size, self.embed_size)
            self.embed_res = nn.Embedding(self.vocab_size, self.embed_size)
    ...
    def forward(self, x_act, x_res):
        ...
        x_act = torch.clamp(x_act, 0, self.embed_act.num_embeddings - 1)
        x_res = torch.clamp(x_res, 0, self.embed_res.num_embeddings - 1)
