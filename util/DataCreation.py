...
class DataCreation:
    ...
    def pad_data(self, data):
        padded_data = []
        max_len = int(self.max_prefix_length)

        # 获取安全的词汇表大小
        vocab_act = self.vocab_size[0] if hasattr(self, 'vocab_size') else 1000
        vocab_res = self.vocab_size[1] if hasattr(self, 'vocab_size') else 1000

        for seq in data:
            # 处理空序列
            if not isinstance(seq, (list, torch.Tensor)) or len(seq) == 0:
                seq = [0]
            ...
            for v in seq:
                try:
                    num = int(v)
                    num = max(0, min(num, vocab_act - 2, vocab_res - 2))
                except:
                    num = 0
                safe_seq.append(num)

            # 转张量
            tensor_seq = torch.tensor(safe_seq, dtype=torch.long)

            # 填充到最大长度
            pad_len = max_len - tensor_seq.shape[0]
            if pad_len > 0:
                tensor_seq = nn.ConstantPad1d((0, pad_len), 0)(tensor_seq)

            padded_data.append(tensor_seq)

        # 🔥 关键：必须 return，永远不会返回 None
