...
class CCHVAE(RecourseMethod):
    ...
    def __init__(self, mlmodel: MLModel, hyperparams: Dict = None) -> None:
        ...
        # With the following line, we are calling the constructor of the RecourseMethod class and passing the mlmodel argument to it.
        # you can find the initialization of self._mlmodel in the file carla/recourse_methods/api/recourse_method.py
        # Super() is a built-in Python function that returns a temporary object of the superclass, which allows you to call its methods.
        # In this case, super() is used to access the methods and attributes of the parent class (RecourseMethod).
        # To conclude, the following line calls the constructor (__init__) of the parent class (RecourseMethod) and passes the mlmodel argument to it. 
        # This means that the parent class's constructor is invoked with the mlmodel argument, allowing the parent class to initialize its own attributes and perform any necessary setup.
        ...
        #this loads (or trains) the generative VAE model
