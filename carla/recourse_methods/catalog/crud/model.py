...
class CRUD(RecourseMethod):
    ...
    def __init__(self, mlmodel, hyperparams: Dict = None):
        ...
        self._vocab_size = checked_hyperparams["vocab_size"]
        self._max_prefix_length = checked_hyperparams["max_prefix_length"]
        vae_params = checked_hyperparams["vae_params"]
        self._csvae = CSVAE(
            checked_hyperparams["data_name"],
            vae_params["layers"],
            self._vocab_size,
            self._max_prefix_length
        )

        if vae_params["train"]:
            self._csvae.fit(
                data= [mlmodel.data.df_train, mlmodel.data.target_train],
            ...
            try:
                self._csvae.load(vae_params['layers'][0])
