...
def check_counterfactuals(
    ...
    """
    Takes the generated array of counterfactuals and checks if these samples are able
    to flip the label from 0 to 1. Every counterfactual which still has a negative label
    will be replaced with an empty row (NaN values).
    ...
    counterfactuals:
        NumPy array of generated samples from recourse methods with shape [batch_size, sequence_length, vocab_size].
    negative_label:
    ...
    predictions = mlmodel.predict(counterfactuals)

    # Find indices where the label is negative
    invalid_indices = np.where(predictions == negative_label)[0]

    # Replace invalid counterfactuals with NaN values
    for idx in invalid_indices:
        counterfactuals[idx] = np.nan
